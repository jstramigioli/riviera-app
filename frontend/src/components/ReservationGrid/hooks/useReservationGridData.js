...
function getDaysArray(start, end) {
  console.log('🔍 DEBUG getDaysArray');
  ...
  while (dt <= end) {
    ...
    arr.push(newDate);
    console.log('📅 Agregando día:', newDate.toISOString());
    dt = addDays(dt, 1);
  }
  
  console.log('📊 Total de días generados:', arr.length);
  console.log('📊 Primeros 5 días:', arr.slice(0, 5).map(d => d.toISOString()));
  console.log('📊 Últimos 5 días:', arr.slice(-5).map(d => d.toISOString()));
